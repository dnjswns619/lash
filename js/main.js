(() => {
  const mainInfo = [
    ...
    {
      type: 'normal',
      scrollHeight: document.querySelector('.slide').offsetHeight,
      objs: {
        container: document.querySelector('.slide'),
      },
      value: {
        marginRight: [0, 75, {start: 0.1, end: 0.83}]
      }
    },
  ];
  const sceneInfo = [
  ...
  function setLayout() {
    for (let i = 0; i < sceneInfo.length; i++) {
      if (sceneInfo[i].type === 'normal') {
        ...
        sceneInfo[i].scrollHeight = sceneInfo[i].objs.container.offsetHeight * 5;
  ...
  function playAnimation() {
    ...
    if (scrollRatio >= 0.83) {
      ...
      sceneInfo[0].objs.container.style.paddingTop = 0;
      document.querySelector('.main').classList.remove('static');
    }
  }
